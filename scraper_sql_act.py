...
if __name__=='__main__':
    ...
    conn = psycopg2.connect('host=ac-transit.cupreqhyyagj.us-west-1.redshift.amazonaws.com port=5439 dbname=act user={} password={}'.format(credentials['user'], credentials['password']))
    ...
    while True:
        if datetime.now() > (last_query_time + timedelta(seconds=wait_secs) or low_trip_count):
            try:
                ...
                response = api.get_vehicle_predictions( "AC" )
